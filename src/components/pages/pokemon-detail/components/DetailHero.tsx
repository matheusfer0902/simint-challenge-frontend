...
export function DetailHero({ pokemon, onBack, onEditClick }: DetailHeroProps) {
  ...
  const canEdit =
    pokemon.creatorId != null &&
    user != null &&
    pokemon.creatorId === user.id;
      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6">
        <div className="mb-6 flex flex-wrap items-center gap-2">
          <button
            type="button"
            onClick={onBack}
            className="flex items-center gap-2 rounded-xl bg-white/20 px-4 py-2 text-sm font-semibold text-white backdrop-blur-sm transition-all hover:bg-white/30 active:scale-[0.97]"
          >
